...
<html>
	...
	<body>
		<form action="proedit" method="post" enctype="multipart/form-data">
			<p class="path">用膳外卖后台管理系统/菜品中心/编辑菜品</p>
			<table border="0" cellspacing="0" cellpadding="0" class="tb">
				<tr>
					<td class="lable">类别<%=objbyid.get("typeid")%></td><td>

					    <select name="typeid">
					    	<option <%=(objbyid.get("typeid").toString().equals("1"))?"selected=\"selected\"":""%> value="1">炒菜</option>
					    	<option <%=(objbyid.get("typeid").toString().equals("2"))?"selected=\"selected\"":""%> value="2">爆炒</option>
					    	<option <%=(objbyid.get("typeid").toString().equals("3"))?"selected=\"selected\"":""%> value="3">焖煮</option>
					    	<option <%=(objbyid.get("typeid").toString().equals("4"))?"selected=\"selected\"":""%> value="4">清蒸</option>
					    	<option <%=(objbyid.get("typeid").toString().equals("5"))?"selected=\"selected\"":""%> value="5">红烧</option>
					    	<option <%=(objbyid.get("typeid").toString().equals("6"))?"selected=\"selected\"":""%> value="6">卤煮</option>
					    	<option <%=(objbyid.get("typeid").toString().equals("7"))?"selected=\"selected\"":""%> value="7">清炒</option>
					    	<option <%=(objbyid.get("typeid").toString().equals("8"))?"selected=\"selected\"":""%> value="8">清炖</option>
					    	<option <%=(objbyid.get("typeid").toString().equals("9"))?"selected=\"selected\"":""%> value="9">干锅</option>
					    </select>
				    </td>
				</tr>
				...
				<tr><td class="lable">产地</td><td><input type="text" name="kind" value="<%=objbyid.get("kind")%>"/></td></tr>
